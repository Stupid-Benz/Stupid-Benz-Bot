...
async def rank(ctx, *, member : discord.Member=None):
	with open('./user.json', 'r') as f:
		users = json.load(f)
	if member == None:
		await ctx.send('```{0}\'s Exp: {1}, Levels: {2}, Next Level Need: {3}```'.format(users[ctx.author.name]['name'], users[ctx.author.name]['exp'], users[ctx.author.name]['lvl'], users[ctx.author.name]['needExp']))
	else:
		if not member.name in users:
			await ctx.send('```That people didn\'t say anything!```')
		else:
			await ctx.send('```{0}\'s Exp: {1}, Levels: {2}, Next Level Need: {3}```'.format(users[member.name]['name'], users[member.name]['exp'], users[member.name]['lvl'], users[member.name]['needExp']))

@bot.command(name='level', help='Get all the members rank')
async def level(ctx):
	await ctx.send('Here is KCCS Official\'s leaderboard: https://KCCS-Official-Rank--stupidbenz.repl.co')
...
async def check(ctx):
	...
	await dm.send('```Balance: {0}```'.format(users[ctx.author.name]['balances']))
...
async def sell(ctx, name, price):
  ...
  if (users[ctx.author.name]['balances'] - balances) >=0:
    ...
    await channel.send('```Name: {0}\nPrice: {1}\nID: {2}\nSeller: {3}\nStatus: Not taken```'.format(name, price, id, ctx.author.name))
    author = bot.get_user(ctx.author.id)
    await author.send('```Name: {0}\nPrice: {1}\nID: {2}\nSeller: {3}\nStatus: Not taken```\n```Balance: {4}```'.format(name, price, id, ctx.author.name, balances))
  else:
    author = bot.get_user(ctx.author.id)
    await author.send('```Balance: {0}```'.format(users[ctx.author.name]['balances']))
    ...
    if id in product:
      if product[id]['status']:
        ...
        if int(price) > users[ctx.author.name]['balances']:
          ...
          await channel.send('```Name: {0}\nPrice: {1}\nID: {2}\nSeller: {3}\nStatus: Taken```'.format(name, price, id, sellerName))
          seller = bot.get_user(sellerID)
          await seller.send('```{0} bought {1}\nBalance: {2}```'.format(ctx.author.name, name, users[sellerName]['balances']))
...
async def shop(ctx):
    ...
    for data in membersdata:
        if data['status'] == True:
            alldata += 'Name: {0}\nPrice: {1}\nSeller: {2}\nID: {3}\n\n'.format(data['name'], data['price'], data['seller'], data['id'])
    alldata += '```'
    if alldata == '``````':
        alldata = 'No data!'
...
async def sellrm(ctx, id):
    ...
    aid = ctx.author.id
    if id in product:
      if aid == product[id]['sellerID'] or aid == 623823202073706496 or aid == 664644679232520233 or aid == 653086042752286730 or aid == 589760724323139631 or aid == 666186125026525194:
        product[id]['status'] = False
        await ctx.send('```Product Ended```')
        channel = bot.get_channel(724101392679174227)
        await channel.send('```Name: {0}\nPrice: {1}\nID: {2}\nSeller: {3}\nStatus: Taken```'.format(product[id]['name'], product[id]['price'], product[id]['id'], product[id]['sellerName']))
      else:
        await ctx.send('```Product doesn\'t exist```')
    else:
...
@bot.command(name='add', help='This command is Moderators could use')
...
@bot.command(name='del', help='This command is Moderators could use')
@commands.has_any_role(664411666154520576, 704252490354262086)
...
@bot.command(name='chka', help='This command is Moderators could use')
@commands.has_any_role(664411666154520576, 704252490354262086)
async def chka(ctx, member: discord.User = None):
	if ctx.channel.id == 704958958950940702:
		...
		if member == None:
			...
			for data in membersdata:
				if data['balances'] != 0:
					alldata += 'Name: {0}\nBalance: {1}\n'.format(data['name'], data['balances'])
			alldata += '```'
			if alldata == '``````':
				alldata = 'No data!'
			await ctx.send(alldata)
		else:
			await ctx.send('```Name: {0}, Balance: {1}```'.format(member.name, users[member.name]['balances']))
	else:
