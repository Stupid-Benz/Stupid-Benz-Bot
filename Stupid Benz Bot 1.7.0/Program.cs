...
namespace Stupid_Benz_Bot
{
    class Program
    {
        ...
        private async Task ReactionAdded(Cacheable<IUserMessage, ulong> argmessage, ISocketMessageChannel channel, SocketReaction reaction)
        {
            var message = await argmessage.DownloadAsync() as IUserMessage;
            if (message.Author != Client.CurrentUser && message.Content != "")
            {
                return;
            }
